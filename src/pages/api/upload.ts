...
cloudinary.config({
  cloud_name: "dxcm9sr8w",
  api_key: "419288487227523",
  api_secret: import.meta.env.CLOUDINARY_API,
});

const ouputDir = path.join(process.cwd(), "public/text");
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const text = data
    .map((blocks: { textAnnotations: { description: string }[] }) => {
      ...
      const content = firstAnnotation["description"] ?? "";
      return content.trim();
    })
    .filter(Boolean)
    .join("\n");

  // TODO: Save text to a database
  fs.writeFile(`${ouputDir}/${id}.txt`, text, "utf-8");
